...
A `Secret` contains the TLS configuration, and username/password or bearer token for authentication as the Service Broker is secure by default.
Username and password is the default authentication mechanism that will be used.
